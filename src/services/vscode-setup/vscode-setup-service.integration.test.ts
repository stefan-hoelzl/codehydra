...
describe("VscodeSetupService Integration", () => {
  ...
  async function createMockAssets(assetsDir: string): Promise<void> {
    ...
    // manifest.json - generated by build:extensions (new flat array format)
    await writeFile(
  ...
  describe("Extension installation failures", () => {
    it("fails with clear error when vsix file is missing", async () => {
      // Clean up assets directory and recreate without the opencode vsix
      ...
      await writeFile(
        ...
        JSON.stringify([
          ...
          },
          {
            id: "sst-dev.opencode",
            version: "0.0.13",
            vsix: "sst-dev-opencode-0.0.13.vsix", // This file won't exist
          },
        ])
      );
      ...
      if (!result.success) {
        ...
        expect(result.error.message).toContain("sst-dev-opencode-0.0.13.vsix");
