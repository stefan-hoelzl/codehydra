...
// Mock getThrottleLevel before imports
let mockThrottleLevel: "off" | "basic" | "full" = "off";

vi.mock("../throttling", () => ({
  getThrottleLevel: () => mockThrottleLevel,
}));
...
describe("ViewManager", () => {
  ...
  describe("Throttling", () => {
    beforeEach(() => {
      // Reset throttle level to 'off' before each test
      mockThrottleLevel = "off";
    });

    describe("throttleView with level=off", () => {
      it("does not call setBackgroundThrottling when level=off", async () => {
        mockThrottleLevel = "off";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        // Activate then deactivate to trigger throttle
        manager.setActiveWorkspace("/path/to/workspace");
        workspaceView.webContents.setBackgroundThrottling.mockClear();

        // Deactivate (this triggers throttleView internally)
        manager.setActiveWorkspace(null);

        // Wait for any async operations
        await new Promise((resolve) => setTimeout(resolve, 10));

        // setBackgroundThrottling should NOT be called with level=off
        expect(workspaceView.webContents.setBackgroundThrottling).not.toHaveBeenCalled();

        manager.destroy();
      });

      it("does not call executeJavaScript when level=off", async () => {
        mockThrottleLevel = "off";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        manager.setActiveWorkspace("/path/to/workspace");
        workspaceView.webContents.executeJavaScript.mockClear();

        manager.setActiveWorkspace(null);

        await new Promise((resolve) => setTimeout(resolve, 10));

        expect(workspaceView.webContents.executeJavaScript).not.toHaveBeenCalled();

        manager.destroy();
      });
    });

    describe("throttleView with level=basic", () => {
      it("calls setBackgroundThrottling(true) when level=basic", async () => {
        mockThrottleLevel = "basic";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        });
        ...
        expect(workspaceView.webContents.setBackgroundThrottling).toHaveBeenCalledWith(true);

        manager.destroy();
      });

      it("calls executeJavaScript with visibilitychange script when level=basic", async () => {
        mockThrottleLevel = "basic";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        expect(workspaceView.webContents.executeJavaScript).toHaveBeenCalledWith(
    ...
    describe("throttleView with level=full", () => {
      it("calls setBackgroundThrottling(true) when level=full", async () => {
        mockThrottleLevel = "full";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        });
      ...
      it("calls executeJavaScript with visibilitychange AND loseContext when level=full", async () => {
        mockThrottleLevel = "full";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        // Should have visibilitychange AND loseContext scripts
        const calls = workspaceView.webContents.executeJavaScript.mock.calls;
        const allScripts = calls.map((c: unknown[]) => c[0]).join(" ");
        expect(allScripts).toContain("visibilitychange");
        expect(allScripts).toContain("loseContext");

        manager.destroy();
      });
    });

    describe("unthrottleView with level=off", () => {
      it("does not call setBackgroundThrottling when level=off", async () => {
        mockThrottleLevel = "off";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
        ...
        // setBackgroundThrottling should NOT be called on view being activated
        expect(workspaceView2.webContents.setBackgroundThrottling).not.toHaveBeenCalled();

        manager.destroy();
      });
    });

    describe("unthrottleView with level=basic", () => {
      it("calls setBackgroundThrottling(false) when level=basic", async () => {
        mockThrottleLevel = "basic";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        // Activate first, then switch to second (which was previously throttled)
        manager.setActiveWorkspace("/path/to/workspace1");
        manager.setActiveWorkspace(null); // Throttle workspace1

        await new Promise((resolve) => setTimeout(resolve, 10));

        workspaceView1.webContents.setBackgroundThrottling.mockClear();

        // Reactivate workspace1 (should unthrottle)
        manager.setActiveWorkspace("/path/to/workspace1");

        // Wait for unthrottle
        await new Promise((resolve) => setTimeout(resolve, 10));

        expect(workspaceView1.webContents.setBackgroundThrottling).toHaveBeenCalledWith(false);

        manager.destroy();
      });

      it("calls executeJavaScript with visibility restore script when level=basic", async () => {
        mockThrottleLevel = "basic";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        // Should restore visibility
        const calls = workspaceView.webContents.executeJavaScript.mock.calls;
        const allScripts = calls.map((c: unknown[]) => c[0]).join(" ");
        expect(allScripts).toContain("visible");

        manager.destroy();
      });
    });

    describe("unthrottleView with level=full", () => {
      it("calls executeJavaScript with restoreContext when level=full", async () => {
        mockThrottleLevel = "full";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        const calls = workspaceView.webContents.executeJavaScript.mock.calls;
        const allScripts = calls.map((c: unknown[]) => c[0]).join(" ");
        expect(allScripts).toContain("restoreContext");

        manager.destroy();
      });
    });

    describe("throttle operation cancellation", () => {
      it("cancels previous throttle operation when new one starts", async () => {
        mockThrottleLevel = "full";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        // Make executeJavaScript slow to allow cancellation
        workspaceView.webContents.executeJavaScript = vi.fn().mockImplementation(() => {
        ...
        // Rapid deactivate/reactivate sequence
        manager.setActiveWorkspace(null);
        manager.setActiveWorkspace("/path/to/workspace");
        manager.setActiveWorkspace(null);
        manager.setActiveWorkspace("/path/to/workspace");
        ...
        // Should not throw and final state should be correct
        expect(manager.getActiveWorkspacePath()).toBe("/path/to/workspace");
    ...
    describe("throttleView error handling", () => {
      it("logs errors from executeJavaScript but does not throw", async () => {
        mockThrottleLevel = "basic";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        workspaceView.webContents.executeJavaScript = vi
          .fn()
    ...
    describe("Integration: workspace switch with throttling=full", () => {
      it("throttles previous workspace and unthrottles new workspace on switch", async () => {
        mockThrottleLevel = "full";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        const ws1 = MockWebContentsViewClass.mock.results[1]!.value;
        const ws2 = MockWebContentsViewClass.mock.results[2]!.value;
        const ws3 = MockWebContentsViewClass.mock.results[3]!.value;

        // Activate first workspace
        manager.setActiveWorkspace("/path/to/ws1");
        await new Promise((resolve) => setTimeout(resolve, 20));

        // Switch to second - first should be throttled
        ws1.webContents.setBackgroundThrottling.mockClear();
        ws2.webContents.setBackgroundThrottling.mockClear();

        manager.setActiveWorkspace("/path/to/ws2");
        await new Promise((resolve) => setTimeout(resolve, 20));

        // ws1 should be throttled (setBackgroundThrottling(true))
        expect(ws1.webContents.setBackgroundThrottling).toHaveBeenCalledWith(true);
        // ws2 should be unthrottled (setBackgroundThrottling(false))
        expect(ws2.webContents.setBackgroundThrottling).toHaveBeenCalledWith(false);

        // Switch to third - second should be throttled
        ws2.webContents.setBackgroundThrottling.mockClear();
        ws3.webContents.setBackgroundThrottling.mockClear();

        manager.setActiveWorkspace("/path/to/ws3");
        await new Promise((resolve) => setTimeout(resolve, 20));

        // ws2 should be throttled
        expect(ws2.webContents.setBackgroundThrottling).toHaveBeenCalledWith(true);
        // ws3 should be unthrottled
        expect(ws3.webContents.setBackgroundThrottling).toHaveBeenCalledWith(false);

        manager.destroy();
      });
    });

    describe("Integration: rapid switching with throttling=full", () => {
      it("handles rapid workspace switching without errors", async () => {
        mockThrottleLevel = "full";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        // Rapid switching 10 times
        const paths = ["/path/to/ws1", "/path/to/ws2", "/path/to/ws3"];
        for (let i = 0; i < 10; i++) {
        ...
        // Final state should be correct (last switch was to ws1, since 10 % 3 = 1 -> index 0 + 1 = ws2... wait let me check)
        // i=0 -> paths[0] = ws1
        // i=1 -> paths[1] = ws2
        // i=2 -> paths[2] = ws3
        // i=3 -> paths[0] = ws1
        ...
        // All workspaces should still exist (no exceptions)
        expect(manager.getWorkspaceView("/path/to/ws1")).toBeDefined();
        expect(manager.getWorkspaceView("/path/to/ws2")).toBeDefined();
        expect(manager.getWorkspaceView("/path/to/ws3")).toBeDefined();
      ...
      it("cancels intermediate throttle operations during rapid switching", async () => {
        mockThrottleLevel = "full";
        const manager = ViewManager.create(mockWindowManager as unknown as WindowManager, {
          uiPreloadPath: "/path/to/preload.js",
          codeServerPort: 8080,
        ...
        // Make executeJavaScript slow
        ws1.webContents.executeJavaScript = vi.fn().mockImplementation(() => {
          return new Promise((resolve) => setTimeout(() => resolve(true), 100));
        });

        // Activate, deactivate, activate rapidly
        manager.setActiveWorkspace("/path/to/ws1");
        manager.setActiveWorkspace("/path/to/ws2");
        manager.setActiveWorkspace("/path/to/ws1");
        manager.setActiveWorkspace("/path/to/ws2");
        ...
        // Should have started multiple operations but cancelled intermediate ones
        // Final state should be correct
        expect(manager.getActiveWorkspacePath()).toBe("/path/to/ws2");
